...
//    LOG(uni::logging::Level::INFO, "header bytes sent: " + bytes_transferred)
    boost::asio::async_write(_socket, boost::asio::buffer(message),
        [this](const boost::system::error_code&, size_t bytes_transferred) {
//      LOG(uni::logging::Level::INFO, "body bytes sent: " + bytes_transferred)
      std::unique_lock<std::mutex> lock(_queue_lock);
      _message_queue.pop();
...
//    LOG(uni::logging::INFO, "header bytes recieved: " + bytes_transferred)
    if (bytes_transferred == 0) {
//      LOG(uni::logging::INFO, "remove socket closed")
      free(header_buf);
      _close_callback();
    } else {
